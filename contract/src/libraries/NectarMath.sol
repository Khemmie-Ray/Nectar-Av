...
library NectarMath {
    ...
    function perMemberTotal(uint256 targetAmount, uint16 numMembers) internal pure returns (uint256) {
    ...
    function baseContribution(uint256 perMember, uint16 totalCycles) internal pure returns (uint256) {
    ...
    function lateJoinerRate(uint256 perMember, uint16 cyclesRemaining) internal pure returns (uint256) {
        require(cyclesRemaining > 0, "NectarMath: zero remaining cycles");
        return perMember / cyclesRemaining;
    }
    ...
    function finalCycleAmount(uint256 perMember, uint256 ratePerCycle, uint16 previousCycles)
        internal
        pure
        returns (uint256)
    {
    ...
    function isWithinTwoXCap(uint256 lateRate, uint256 baseRate) internal pure returns (bool) {
        return lateRate < baseRate * 2;
    }

    /// @notice Safeguard 2: Enrollment closes when fewer than 3 cycles remain.
    function isAboveThreeCycleFloor(uint16 cyclesRemaining) internal pure returns (bool) {
        return cyclesRemaining >= 3;
    }
    ...
    function enrollmentWindowCycles(uint16 totalCycles, uint8 windowType) internal pure returns (uint16) {
        if (windowType == 0) {
            return totalCycles / 2;
        } else if (windowType == 1) {
            uint16 strictCycles = totalCycles / 4;
            return strictCycles == 0 ? 1 : strictCycles;
    ...
    function isWithinEnrollmentWindow(uint16 currentCycle, uint16 totalCycles, uint8 windowType)
        internal
        pure
        returns (bool)
    {
        uint16 openCycles = enrollmentWindowCycles(totalCycles, windowType);
        return openCycles > 0 && currentCycle <= openCycles;
    }

    // ─── Minimum Fill Threshold ───────────────────────────────────────────────

    /// @notice Pool requires >= 50% fill AND >= 3 members at enrollment close.
    function meetsMinFillThreshold(uint16 activeMembers, uint16 maxMembers) internal pure returns (bool) {
    ...
    function adjustedWinnerCount(uint16 configuredWinners, uint16 activeMembers) internal pure returns (uint16) {
    ...
    function computeCurrentCycle(uint256 startTimestamp, uint256 currentTimestamp, uint32 cycleDuration)
        internal
        pure
        returns (uint16)
    {
    ...
    function remainingCycles(uint16 current, uint16 total) internal pure returns (uint16) {
