...
contract NectarMathTest is Test {
    ...
    uint256 constant TARGET_WEEKLY = 12_000 * USDC_UNIT; // 12,000 USDC
    uint16 constant MEMBERS = 6;
    uint16 constant CYCLES_WEEKLY = 10; // 10 weeks

    uint256 constant TARGET_DAILY = 600 * USDC_UNIT; // 600 USDC
    uint16 constant MEMBERS_DAILY = 10;
    uint16 constant CYCLES_DAILY = 30; // 30 days

    uint256 constant TARGET_MONTHLY = 60_000 * USDC_UNIT; // 60,000 USDC
    ...
    function test_BaseContribution_WeeklyPool() public pure {
        ...
        assertEq(perMember, 2_000 * USDC_UNIT, "Per-member total wrong");
        assertEq(base, 200 * USDC_UNIT, "Base contribution wrong");
    }
    ...
    function test_BaseContribution_DailyPool() public pure {
        ...
        assertEq(perMember, 60 * USDC_UNIT, "Per-member total wrong");
        assertEq(base, 2 * USDC_UNIT, "Base contribution wrong (should be 2 USDC/day)");
    }
    ...
    function test_BaseContribution_MonthlyPool() public pure {
        ...
        assertEq(perMember, 3_000 * USDC_UNIT, "Per-member total wrong");
        assertEq(base, 500 * USDC_UNIT, "Base contribution wrong");
    }
    ...
    function test_LateJoiner_Week4_Of10() public pure {
        ...
        assertEq(rate, 285_714_285, "Late joiner week 4 rate wrong");
    }
    ...
    function test_LateJoiner_Day10_Of30() public pure {
        ...
        assertEq(rate, 2_857_142, "Late joiner day 10 rate wrong");
    }
    ...
    function test_LateJoiner_Month2_Of6() public pure {
        ...
        assertEq(rate, 600 * USDC_UNIT, "Late joiner month 2 rate wrong (3000/5=600)");
    }
    ...
    function test_TwoXCap_Passes_Week5() public pure {
        uint256 baseRate = 200 * USDC_UNIT; // 200 USDC/week
        uint256 lateRate = 333 * USDC_UNIT; // Joining week 5 gives 2000/6 ≈ 333 USDC/week
    ...
    function test_TwoXCap_Fails_Week6_Exactly() public pure {
        uint256 baseRate = 200 * USDC_UNIT;
        uint256 lateRate = 400 * USDC_UNIT; // Exactly 2x — should be blocked per PRD
        bool allowed = NectarMath.isWithinTwoXCap(lateRate, baseRate);
    ...
    function test_TwoXCap_DailyPool_Day16_Blocked() public pure {
        ...
        bool allowed = NectarMath.isWithinTwoXCap(rate, 2 * USDC_UNIT);
        assertFalse(allowed, "Day 16 (4 USDC/day = exactly 2x) should be blocked");
    }
    ...
    function test_TwoXCap_MonthlyPool_Month4_Blocked() public pure {
        ...
        bool allowed = NectarMath.isWithinTwoXCap(rate, 500 * USDC_UNIT);
    ...
    function test_EnrollmentWindow_Strict_MinimumPool_Week1_Open() public pure {
        bool open = NectarMath.isWithinEnrollmentWindow(1, 3, 1);
        assertTrue(open, "Strict window should leave at least cycle 1 open");
    }

    function test_EnrollmentWindow_Strict_MinimumPool_Week2_Closed() public pure {
        bool open = NectarMath.isWithinEnrollmentWindow(2, 3, 1);
        assertFalse(open, "Strict window on 3 cycles should close after cycle 1");
    }
    ...
    function test_FinalCycleRounding_HitsExactTotal() public pure {
        ...
        uint256 perMember = 2_000 * USDC_UNIT;
    ...
    function test_ProtocolFee_300USDC_Yield() public pure {
        uint256 totalYield = 300 * USDC_UNIT;
        uint256 fee = NectarMath.protocolFee(totalYield);
        uint256 winnersShare = NectarMath.winnersShare(totalYield);
        assertEq(fee, 15 * USDC_UNIT, "Protocol fee should be 15 USDC");
        assertEq(winnersShare, 285 * USDC_UNIT, "Winners share should be 285 USDC");
    ...
    function testFuzz_LateJoinerRate_NeverExceedsTotal(uint256 perMember, uint16 remainingCycles) public pure {
        vm.assume(perMember > 0 && perMember < 1_000_000_000 * USDC_UNIT);
    ...
    function testFuzz_FeeAndShare_SumToTotal(uint256 totalYield) public pure {
        vm.assume(totalYield < 100_000_000 * USDC_UNIT);
        uint256 fee = NectarMath.protocolFee(totalYield);
