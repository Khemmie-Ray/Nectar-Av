...
contract NectarPoolTest is Test {
    ...
    uint256 constant RATE = 200 * USDC_UNIT;
    ...
    MockERC20 token;
    MockAavePool aave;
    MockVRFModule vrfModule;
    NectarPool blueprint;
    NectarFactory factory;

    // ─── Pool Constants ──────────────────────────────────────────────────────
    uint256 constant TARGET = 12_000 * USDC_UNIT; // 12,000 USDC
    ...
    function setUp() public {
        // Deploy mocks
        token = new MockERC20("Mock USDC", "mUSDC", USDC_DECIMALS);
        aave = new MockAavePool();

        // Deploy the blueprint and factory
        blueprint = new NectarPool();
        factory = new NectarFactory(address(blueprint), address(aave), address(token), address(0), treasury);
        vrfModule = new MockVRFModule();
        factory.setVrfModule(address(vrfModule));
        token.mint(address(aave), 1_000_000 * USDC_UNIT);

        // Fund test actors
        address[6] memory actors = [creator, alice, bob, carol, dave, eve];
        for (uint256 i = 0; i < actors.length; i++) {
            token.mint(actors[i], 10_000 * USDC_UNIT);
    ...
    function test_CreatePool_ConfigStoredCorrectly() public {
        NectarPool pool = _createWeeklyPool();
        (address tkn,, uint16 max, uint16 cycles, uint16 winners, uint32 duration,,) = _unpackConfig(pool);
    ...
    function test_Factory_EnforcesThreeActivePoolsPerMember() public {
        NectarPool[4] memory pools;
        for (uint8 i = 0; i < 4; i++) {
            pools[i] = _createWeeklyPool();
        }
        ...
        vm.prank(alice);
        vm.expectRevert("NectarPool: active pool increment failed");
        pools[3].joinPool(0);

        assertEq(factory.activePoolCount(alice), 3);
    }
    ...
    function test_Factory_RejectsNonUSDCToken() public {
        MockERC20 otherToken = new MockERC20("Other Token", "OTHER", USDC_DECIMALS);
        INectarPool.PoolConfig memory cfg = _baseConfig();
        cfg.token = address(otherToken);
        vm.prank(creator);
        vm.expectRevert("NectarFactory: token must be USDC");
        factory.createPool(cfg);
    }

    // ─── 2. JoinPool Tests ────────────────────────────────────────────────────

    function test_JoinPool_AcceptsUser() public {
        NectarPool pool = _createWeeklyPool();
    ...
    function test_JoinPool_StrictThreeCyclePool_AllowsCycleOneOnly() public {
        INectarPool.PoolConfig memory cfg = _baseConfig();
        cfg.totalCycles = 3;
        cfg.enrollmentWindow = INectarPool.EnrollmentWindow.STRICT;

        vm.prank(creator);
        NectarPool pool = NectarPool(factory.createPool(cfg));
        ...
        vm.warp(pool.poolStartTime() + WEEKLY + 1);
        vm.prank(bob);
        token.approve(address(pool), type(uint256).max);
        vm.expectRevert("NectarPool: enrollment window closed");
        pool.joinPool(0);
    ...
    function test_JoinPool_LateJoiner_RecalculatedRate() public {
        ...
        // Expected: 2,000 USDC / 7 remaining cycles
        ...
        assertEq(rate, expectedRate, "Late joiner rate should be 2000/7");
    }
    ...
    function test_Deposit_AcceptsCorrectAmount() public {
        NectarPool pool = _createWeeklyPool();
        uint256 rate = RATE; // 200 USDC base rate
    ...
    function test_Deposit_RejectsWrongAmount() public {
        ...
        pool.deposit(RATE + 1); // 1 base unit too much
    }
    ...
    function test_EmergencyWithdraw_DecrementsActivePoolCount() public {
        NectarPool pool = _createWeeklyPool();
        vm.prank(alice);
        pool.joinPool(0);
        assertEq(factory.activePoolCount(alice), 1);

        vm.prank(alice);
        pool.emergencyWithdraw();

        assertEq(factory.activePoolCount(alice), 0);
    }
    ...
    function test_CalculateJoinRate_Week1_CanJoin() public {
        ...
        assertEq(rate, RATE, "Base rate for cycle 1");
    ...
    {
        (
            string memory ignoredName, // name (unused in test assertions)
            address t,
            uint256 ta,
            uint16 mm,
            uint16 tc,
        ...
        ignoredName;
        return (t, ta, mm, tc, wc, cd, uint8(ew), uint8(dm));
    }
    ...
    /// @dev Advance beyond the yield period and trigger the draw flow.
    ...
    function test_JoinPool_RejectsWhenFull() public {
        ...
        // frank has funds and approval, but the pool is full
        token.mint(frank, 10_000 * USDC_UNIT);
        vm.prank(frank);
        token.approve(address(pool), type(uint256).max);
    ...
    function test_LazyEvict_PrincipalQueuedForRefund() public {
        ...
        vm.prank(alice);
        pool.joinPool(0); // totalPaid = 200 USDC
    ...
    /// @dev Simulates the full on-chain lifecycle using the vault, mock Aave, and mock VRF.
    function test_FullLifecycle_WinnersAndNonWinnersClaimed() public {
        ...
        // Advance through the yield phase and settle via mock VRF + vault withdrawal.
        _advanceToDrawing(pool);
        assertEq(uint256(pool.state()), uint256(INectarPool.PoolState.SETTLED));

        // Treasury received 5% fee
        uint256 principal = rate * 10 * 3; // 6,000 USDC total from 3 members
        uint256 yield = principal * 500 / 10_000; // MockAave default 5%
        uint256 expectedFee = yield * 5 / 100; // 15 USDC
    ...
    function test_Settlement_ClosesActivePoolCounts() public {
        NectarPool pool = _createWeeklyPool();
        uint256 rate = RATE;
        ...
        pool.joinPool(0);
        assertEq(factory.activePoolCount(alice), 1);
        assertEq(factory.activePoolCount(bob), 1);
        assertEq(factory.activePoolCount(carol), 1);
        ...
        assertEq(factory.activePoolCount(alice), 0);
        assertEq(factory.activePoolCount(bob), 0);
        assertEq(factory.activePoolCount(carol), 0);
    }
    ...
    function testFuzz_JoinRate_Invariants(uint8 elapsedCycles) public {
        ...
        NectarPool pool = _createWeeklyPool();
        uint256 perMember = TARGET / MEMBERS; // 2,000 USDC
        uint256 baseRate = perMember / CYCLES; // 200 USDC
