...
export default function PoolDetails({
  ...
  const stateLabel = poolDetails ? POOL_STATE_LABELS[poolDetails.state] || "Unknown" : "";
  const stateColor = poolDetails ? POOL_STATE_COLORS[poolDetails.state] || "bg-gray-100 text-gray-700" : "";
  ...
  const slotsLeft = poolDetails ? poolDetails.maxMembers - poolDetails.activeMembers : 0;
  ...
  const isUserMember = userMember && userMember.joinCycle > 0 && !userMember.isRemoved;
          ...
          <>
            <div className="flex mb-6 justify-between lg:flex-row md:flex-row flex-col gap-4">
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-6 lg:w-[28%] md:w-[28%] w-full">
                <div className={`bg-linear-to-br ${flower.gradient} rounded-lg p-6 text-center mb-4`}>
                  <span className="text-6xl">{flower.emoji}</span>
                  <p className={`text-sm mt-2 font-semibold ${flower.accentColor}`}>
                    {flower.name}
                  </p>
                </div>
                ...
                <div className="space-y-3 text-xs">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-[#252B36]">{poolDetails.totalCycles} {unit}s</span>
                  </div>
                  <div className="flex justify-between">
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-6 lg:w-[28%] md:w-[28%] w-full">
                <div className="mb-4 pb-4 border-b border-gray-200">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="text-right">
                      <p className="text-2xl font-bold text-[#252B36]">{poolDetails.activeMembers}</p>
                      <p className="text-[10px] text-[#7D7C7C]">Active Members</p>
                    </div>
                  </div>
                ...
                <div>
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className="text-right">
                      <p className="text-2xl font-bold text-[#252B36]">{poolDetails.currentWinnersCount}</p>
                      <p className="text-[10px] text-[#7D7C7C]">Winners Drawn</p>
                    </div>
                  </div>
                  <div className="flex justify-between text-[10px] text-[#7D7C7C]">
                    <span>Target: {poolDetails.winnersCount} {poolDetails.winnersCount === 1 ? "winner" : "winners"}</span>
                    <span>Remaining: {poolDetails.activeMembers - poolDetails.currentWinnersCount} non-winners</span>
                  </div>
                </div>
                    ...
                    {isUserMember ? (
                      <div className="bg-green-50 border border-green-200 rounded-lg p-3 space-y-1">
                        ...
                        <p className="text-green-700 text-[10px]">
                          ...
                          {formatAmount(userMember!.totalPaid)} total deposited
                        </p>
                        <p className="text-green-700 text-[10px]">
                          Rate: {formatAmount(userMember!.assignedRate)} per {unit}
                        </p>
                      </div>
                    ) : isUserRemoved ? (
                      ...
                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                        ...
                        {poolDetails.joinRate && (
                          <p className="text-blue-700 text-[10px]">
                            Your rate: {formatAmount(poolDetails.joinRate)} per {unit}
                          </p>
                        )}
                      </div>
                    ) : (
                      <div className="bg-gray-50 border border-gray-200 rounded-lg p-3">
                        <p className="text-gray-600 text-xs">Enrollment closed or pool full</p>
                      </div>
                    )}
                  </div>
                )}
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-6 lg:w-[40%] md:w-[40%] w-full">
                ...
                <div className="grid grid-cols-3 gap-2 mb-3">
                  <div className="bg-[#252B36] p-3 text-center rounded">
                    <p className="text-white text-sm font-bold mb-0.5">
                      {formatAmount(poolDetails.targetAmount)}
                    </p>
                ...
                <div className="flex justify-between text-[9px] text-[#7D7C7C]">
                  <span>{poolDetails.winnersCount} {poolDetails.winnersCount === 1 ? "winner" : "winners"} split yield</span>
                  <span>{freq} contributions</span>
                </div>

                {isUserMember &&
                  poolDetails.userClaimable !== undefined &&
                  poolDetails.userClaimable > BigInt(0) && (
                    <div className="mt-4 bg-yellow-50 border border-yellow-200 rounded-lg p-3 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-yellow-900 font-bold text-sm">
                          {formatAmount(poolDetails.userClaimable)}
                        </p>
                      </div>
                    </div>
                  )}
            ...
            <div className="flex mb-6 justify-between lg:flex-row md:flex-row flex-col gap-4 lg:gap-0 md:gap-0">
              ...
              <div className="bg-white border border-gray-200 rounded-xl sm:rounded-2xl p-4 sm:p-6 pb-3 sm:pb-4 lg:w-[58%] md:w-[58%] w-full">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2">
                      ...
                      <span className="text-[10px] sm:text-xs text-[#7D7C7C]">
                        Vault: {poolDetails.vault.slice(0, 6)}...{poolDetails.vault.slice(-4)}
                      </span>
                    </div>
                  </div>
                  <div className="text-left sm:text-right w-full sm:w-auto">
                    <p className="text-xs sm:text-sm font-bold text-[#252B36] mb-1.5 sm:mb-2">
                      Total Deposits: ${formatAmount(totalDeposited)}
                    </p>
                    <div className="flex flex-col items-start sm:items-end gap-1">
                      <div className="flex items-center gap-2 text-xs">
                        <div className="w-3 h-3 bg-blue-500 rounded-sm"></div>
                        <span className="text-[#252B36]">
                          Deposits: ${formatAmount(totalDeposited)}
                        </span>
                      </div>
                      <div className="flex items-center gap-2 text-xs">
                        ...
                        <span className="text-[#252B36]">
                          {vaultInfo?.hasActiveDeposit
                            ? `In Aave: $${formatAmount(vaultInfo.principal)}`
                            : "Yield: awaiting deposit"}
                        </span>
                      </div>
                      {vaultInfo?.isDelayed && (
                        <p className="text-[10px] text-orange-500">⚠ Aave withdrawal delayed</p>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="relative h-48 sm:h-56 md:h-64">
                  <YieldChart
                    ...
                    totalDeposited={
                      vaultInfo?.hasActiveDeposit ? vaultInfo.principal : totalDeposited
                    }
                    pendingProfit={BigInt(0)}
                    decimals={6}
                  />
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl sm:rounded-2xl p-4 sm:p-6 pb-3 sm:pb-4 lg:w-[40%] md:w-[40%] w-full my-4 lg:my-0 md:my-0">
                ...
                <PoolActionForm
                  ...
                  perMember={perMember}
                  assignedRate={isUserMember ? userMember!.assignedRate : BigInt(0)}
                  claimableAmount={poolDetails.userClaimable ?? BigInt(0)}
                  totalPaid={isUserMember ? userMember!.totalPaid : BigInt(0)}
                  frequencyUnit={unit}
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-200">
                      <thead className="bg-gray-50 border-b border-gray-200">
                        <tr>
                          <th className="px-3 sm:px-6 py-3 sm:py-4 text-left text-[10px] sm:text-xs font-semibold text-[#252B36]">#</th>
                          <th className="px-3 sm:px-6 py-3 sm:py-4 text-left text-[10px] sm:text-xs font-semibold text-[#252B36]">Address</th>
                          <th className="px-3 sm:px-6 py-3 sm:py-4 text-left text-[10px] sm:text-xs font-semibold text-[#252B36]">Per {unit}</th>
                          <th className="px-3 sm:px-6 py-3 sm:py-4 text-left text-[10px] sm:text-xs font-semibold text-[#252B36]">Total Paid</th>
                          <th className="px-3 sm:px-6 py-3 sm:py-4 text-left text-[10px] sm:text-xs font-semibold text-[#252B36]">Cycles</th>
                          <th className="px-3 sm:px-6 py-3 sm:py-4 text-left text-[10px] sm:text-xs font-semibold text-[#252B36]">Status</th>
                        </tr>
                      </thead>
                      <tbody className="bg-white divide-y divide-gray-200">
                        {paginatedMembers.length > 0 ? (
                          paginatedMembers.map((member: MemberDetail, index: number) => {
                            const globalIndex = (currentPage - 1) * membersPerPage + index;
                            const isCurrentUser =
                              userAddress &&
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="px-3 sm:px-6 py-3 sm:py-4 text-[10px] sm:text-xs text-[#7D7C7C]">
                                  ${formatAmount(member.assignedRate)}
                                </td>
                                <td className="px-3 sm:px-6 py-3 sm:py-4 text-[10px] sm:text-xs text-[#7D7C7C]">
                                  ${formatAmount(member.totalPaid)}
                                </td>
                                <td className="px-3 sm:px-6 py-3 sm:py-4 text-[10px] sm:text-xs text-[#7D7C7C]">
                                  {member.cyclesPaid} / {poolDetails.totalCycles}
                                </td>
                                <td className="px-3 sm:px-6 py-3 sm:py-4 text-[10px] sm:text-xs">
                                  {member.isRemoved ? (
                          ...
                          <tr>
                            <td colSpan={6} className="px-6 py-12 text-center">
                              <div className="flex flex-col items-center gap-2">
                                ...
                                <p className="text-xs text-gray-400">Be the first to join this pool!</p>
                              </div>
                            </td>
                          </tr>
                        )}
                  ...
                  {totalPages > 1 && (
                    <div className="px-3 sm:px-6 py-3 sm:py-4 bg-gray-50 border-t border-gray-200 flex flex-wrap items-center justify-between gap-3">
                      <div className="text-xs text-[#7D7C7C]">
                        ...
                        {Math.min(currentPage * membersPerPage, memberList.length)} of{" "}
